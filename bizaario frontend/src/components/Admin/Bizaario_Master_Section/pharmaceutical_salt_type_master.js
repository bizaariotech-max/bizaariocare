...
function PharmaceuticalSaltTypeMaster() {
  ...
  const[loading,setloading]=useState(false)
     ...
     const [salt_type_master, setsalt_type_master] = useState({
      ...
      const[all_salt_type_master,setall_salt_type_master]=useState([])
      const getall_salt_type_master=async()=>
      {
        try {
          const resp=await api.post('api/v1/admin/LookupList/',{lookupcodes:"pharmaceutical_salt_type"})
          console.log(resp);
          
          setall_salt_type_master(resp.data.data)
  ...
  const onDelete=()=>
  {
    alert("delete")
     ...
     const columnsymptommaster = [
        ...
        { field: 'lookup_value', headerName: 'Pharmaceutical Salt Type', flex: 0.5 }, 
       {
      field: 'actions',
      headerName: 'Actions',
      width: 80,
      ...
      const rowssymptom = all_salt_type_master?.map((doc, index) => ({
        id: doc._id || index,
        ...doc,
      }));
   ...
   <Adminheader />
      ...
      <div className="layout">
        ...
        <div className='profile-header'>
                  <h3>Enter Details for Pharmaceutical Salt Type Master</h3>
                  <p>Add or update the required details for the pharmaceutical salt type master to keep records accurate and complete.</p>
                  </div>
      ...
      <div className="form-grid">
           ...
           <FormControl fullWidth size="small">
            ...
            <TextField 
              name="salt_type"
              defaultValue={salt_type_master.salt_type}
              onChange={handlechange}
              placeholder='salt type'
            >
