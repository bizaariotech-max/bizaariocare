...
export default function AboutContent({ hospitalData }) {
  ...
  const fetchContent = async () => {
    try {
      ...
      const digitalCMEResponse = await api.post("api/v1/admin/ContentList", {
        page: 1,
        limit: 20,
      ...
      if (digitalCMEResponse.data?.data?.list) {
        setDigitalCMEContent(digitalCMEResponse.data.data.list);
      }

      // Fetch Case Study content
      const caseStudyResponse = await api.post("api/v1/admin/ContentList", {
        page: 1,
        limit: 20,
      ...
      if (caseStudyResponse.data?.data?.list) {
        setCaseStudyContent(caseStudyResponse.data.data.list);
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="mt-10 lg:mt-20">
        ...
        {(digitalCMEContent.length > 0 || DigitalCmeArr.length > 0) ? (
          <Carousel
