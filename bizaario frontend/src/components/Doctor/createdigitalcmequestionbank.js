...
export default function Createdititalcmequestionbank() {
  ...
  const handleDeleteImage = (index) => {
    setquestionbank((prev) => ({
      ...prev,
      image_gallary_preview: prev.image_gallary_preview.filter(
        (_, i) => i !== index
      ),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resp = await api.post(
        "/doctor/createdigitalcmequestionbank",
        questionbank,
        {
          headers: {
    ...
    <>
      ...
      <div className="create-question-bank">
        ...
        >
          ...
          <p>
            Add or update the required details for question bank to keep records
                ...
                >
                  ...
                  >
                    {alldigitalcme.map((item) => (
                      <MenuItem
                        key={item.digital_cme_id}
                        value={item.digital_cme_id}
                      >
                        {item.digital_cme_id}
                      </MenuItem>
                    ))}
                    ...
                    <>
                      ...
                      {questionbank.answer_options.map((option, index) => (
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              checked={questionbank.valid_answer.includes(
                                option
                              )}
                  ...
                  {Array.isArray(questionbank?.image_gallary_preview) &&
                    questionbank.image_gallary_preview.length > 0 && (
                      <div>
                        {questionbank.image_gallary_preview.map(
                          (item, index) => (
                            <div
                              key={index}
                              style={{
