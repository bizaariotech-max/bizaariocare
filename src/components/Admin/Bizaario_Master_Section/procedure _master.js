...
function ProcedureMaster() {
  ...
  const[loading,setloading]=useState(false)
     ...
     const [procedure_master, setprocedure_master] = useState({
    ...
    explanation: "",
      ...
      const[all_procedure_master,setall_procedure_master]=useState([])
      const getall_procedure_master=async()=>
      {
        try {
            const resp=await api.post('api/v1/admin/LookupList/',{lookupcodes:"procedure_master"})
          console.log(resp);
          
          setall_procedure_master(resp.data.data)
  ...
  const onDeletehospital=()=>
  {
    alert("delete")
     ...
     const columns = [
        ...
        { field: 'parent_lookup_name', headerName: 'Procedure Group Name', flex: 0.5 }, 
        { field: 'lookup_value', headerName: 'Procedure Name', flex: 0.5 },
      ...
      const rows = all_procedure_master?.map((doc, index) => ({
        id: doc._id || index,
        ...doc,
      }));
  ...
  setprocedure_master((prev) => {
    ...
    return { ...prev, [name]: type === "checkbox" ? checked : value };
        ...
        const add_procedure_master = async () => {
        try {
          setloading(true)
          ...
          if (resp.data.response.response_code === "200") {
              Swal.fire({
                      icon:"success",
                      title:"Procedure Master Added",
                      text:"Procedure Group Master Addedd Successfully...",
 ...
 <Adminheader />
      ...
      <div className="layout">
        ...
        <div className='profile-header'>
                  <h3>Enter Details for Procedure Master</h3>
                  <p>Add or update the required details for the procedure master to keep records accurate and complete.</p>
                  </div>
      ...
      <div className="form-grid">
             ...
             <label className="form-label">Procedure Group Name</label>
            <Select 
             ...
             MenuProps={{
                ...
                renderValue={(selected) => {
                  if (!selected) {
                    return <span style={{ color: "#9ca3af" }}>Procedure Group Name</span>; // grey placeholder
                  }
                  return group_name.find((item) => item._id === selected)?.lookup_value;
                }}
            >
       ...
       <FormControl fullWidth size="small">
            ...
            <TextField 
              name="procedure_name"
              defaultValue={procedure_master.procedure_name}
              onChange={handlechange}
